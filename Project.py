...
ROOT_FOLDER_ID = os.getenv(
    "ROOT_FOLDER_ID", "1M8ho1aDYD3t6q0UPd63Ud7K0RUBndtkW"
)
...
# ตรวจหาค่า JSON จาก Environment Variables ทั้งสองชื่อ
service_account_env = os.getenv("GOOGLE_CREDENTIALS_JSON") or os.getenv(
...
if service_account_env:
  ...
  creds = Credentials.from_service_account_file(
      str(KEY_FILE_PATH), scopes=SCOPES
  )

drive_service = build("drive", "v3", credentials=creds)

# บันทึกสถานะโฟลเดอร์ {user_id: "ชื่อโฟลเดอร์"}
user_company_state = {}
...
# 2. ดักรับรูปภาพแล้วอัปโหลดขึ้น Google Drive
...
def handle_image_message(event: MessageEvent):
  ...
  try:
    ...
    with ApiClient(configuration) as api_client:
      ...
      file_metadata = {"name": f"{message_id}.jpg", "parents": [date_folder_id]}
      media = MediaIoBaseUpload(
          io.BytesIO(image_content), mimetype="image/jpeg"
      )
